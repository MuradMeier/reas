...
export default function TrashPage() {
  ...
  const handleHardDelete = async (type: string, id: number) => {
    ...
    try {
      await api.post(`/trash/hard-delete/${type}/${id}/`);
              ...
              <TableBody>
                {trash.map((item: any) => (
                  <TableRow key={`${item.content_type}-${item.id}`}>
                    ...
                    <TableCell>{item.object_repr}</TableCell>
                    <TableCell>{format(new Date(item.deleted_at), 'dd.MM.yyyy HH:mm', { locale: ru })}</TableCell>
                    <TableCell>{item.deleted_by || '—'}</TableCell>
                    <TableCell className="text-right space-x-2">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleRestore(item.content_type, item.id)}
                      >
                        Восстановить
                      </Button>
                      <Button
                        size="sm"
                        variant="destructive"
                        onClick={() => handleHardDelete(item.content_type, item.id)}
                      >
                        Удалить навсегда
                      </Button>
                    </TableCell>
