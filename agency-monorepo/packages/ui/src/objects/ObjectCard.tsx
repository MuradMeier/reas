...
interface ObjectCardProps {
  item: any; // или ваш тип SearchResult
}

export default function ObjectCard({ item }: ObjectCardProps) {
    console.log('ObjectCard item:', item);
  const formattedDate = item.created_at
    ? format(new Date(item.created_at), 'dd.MM.yyyy', { locale: ru })
    : null;

  // Fallback: сначала item.image, потом item.images[0]?.izobrazhenie
  const imageUrl = item.image || item.images?.[0]?.izobrazhenie || null;
